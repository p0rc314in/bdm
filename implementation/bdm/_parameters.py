        ...
        with torch.no_grad():
            ...
            for role, parameter in (
                ...
                if parameter is not None:
                    ...
                    if role != "initial_state.full":
                        # Two independent additive factors share one row's
                        # variance budget. Scale the existing FP32 draws so
                        # role seeds and all other initial tensors are stable.
                        parameter.mul_(1 / math.sqrt(2))
