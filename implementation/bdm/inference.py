...
from .gdn2_banks import product_key_route
...
def step(layer, hidden, state):
    ...
    q,k,v,g,b,w=layer._controllers(hidden,batch_qk=True)
    read,write=layer._route_projections(hidden)
    if layer.num_writes == layer.num_reads:
        weights,indices=product_key_route(torch.cat((write,read),dim=0),
            bank_count=layer.bank_count,selected_banks=layer.num_writes,batch_factors=True)
        ww,rw=weights.chunk(2,dim=0)
        wi,ri=indices.chunk(2,dim=0)
    else:
        ww,wi=layer._route(write,layer.num_writes)
        rw,ri=layer._route(read,layer.num_reads)
