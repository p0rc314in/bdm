...
class SparseDeltaMemory(SDMStatsMixin, nn.Module):
    ...
    def __init__(self, args: SparseDeltaMemoryArgs, layer_id: int):
        ...
        # Each head independently addresses slots_per_head rows of head_dim values.
        self.slots_per_head = args.slots_per_head

        if self.slots_per_head <= 0:
            raise ValueError('slots_per_head must be positive')
        rows = math.isqrt(self.slots_per_head)
        while self.slots_per_head % rows:
            rows -= 1
        self.product_key_rows = rows
        self.product_key_columns = self.slots_per_head // rows
        self.key_size_per_head = self.product_key_rows + self.product_key_columns
    ...
    def _get_product_keys_for_head(
        ...
        B, T, _ = x.shape
        if self.product_key_rows == self.product_key_columns:
            q1, q2 = x.chunk(2, dim=-1)
        else:
            q1, q2 = x.split((self.product_key_rows, self.product_key_columns), dim=-1)
            q1 = F.pad(q1, (0, self.product_key_columns - self.product_key_rows), value=-float('inf'))
            half_key = self.product_key_columns
