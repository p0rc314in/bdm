...
def gated_sparse_memory_write_read_inference_v2(
    ...
    CW = C * num_writes
    # Signed wrap only permutes groups until the identifier space exceeds
    # 2**32; beyond that it aliases distinct chunks and corrupts decay.
    key_dtype = torch.int64 if num_chunks * num_slots > 2**32 else torch.int32
    chunk_offsets = torch.arange(num_chunks, device=device, dtype=key_dtype).unsqueeze(1) * num_slots
    sort_keys = (flat_k.to(key_dtype) + chunk_offsets).reshape(-1)
...
def _fused_triple_cumsum_kernel(
    ...
    # The chunk-by-slot scratch table can exceed 2**31 elements.
    accum_fwd_base = bid.to(tl.int64) * num_slots
    accum_rev_base = bid.to(tl.int64) * num_slots
    k_base = bid * C_dim * W_dim
    q_base = bid * C_dim * R_dim
    w_offs = tl.arange(0, BLOCK_W)
    r_offs = tl.arange(0, BLOCK_R)
