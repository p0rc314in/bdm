    ...
    if cid < tl.load(CHOFF + NSEQ):
        ...
        for half in range(1, -1, -1):
            ...
            if half_begin < window_end:
                if chunk == 0:
                    hhalf = tl.load(INIT + (sequence % INIT_SEQS) * BK * D + rows[:, None] * D + values[None, :], mask, 0).to(tl.float32)
                else:
                    hhalf = tl.load(SNAP + (first + chunk - 1) * BK * D + rows[:, None] * D + values[None, :], mask, 0).to(tl.float32)
